...
- Cannot use only vx in controller

## 2. Info

- realsense: https://github.com/IntelRealSense/realsense-ros/tree/ros1-legacy
- roslaunch realsense2_camera rs_camera.launch serial_no:=801212070256 filters:=pointcloud

## 3. Preparations

### 3.1 Install Real Sense Driver
...
```bash
export ROS_MASTER_URI=http://192.168.7.163:11311
export ROS_IP=192.168.7.92
```
...
```bash
export ROS_MASTER_URI=http://192.168.7.163:11311
export ROS_IP=192.168.7.163
```

## 5. Start With Seperate Programs: Common

### 5.1 Roscore on Backpack

```bash
roscore
```

### 5.2 Start dji sdk on NUC

SDK:
```bash
roslaunch lrs_launch sdk_nuc.launch m600:=false ns:=/dji2
```
...
```bash
rosservice call /dji2/dji_sdk/drone_task_control "task: 4"
```
### 5.5 Echo Local Position

```bash
rostopic echo /dji2/dji_sdk/local_position
```

## 6. Start With Seperate Programs: Only Simulator

### 6.1 Use local pos

```bash
rosservice call /dji2/dji_sdk/set_local_pos_ref "{}"
```

### 6.2 Start C++ Controller (Use this!)

```bash
rosrun lrs_fly_with_vicon controller __ns:=/dji2 _vicon:=false _mpc:=false _start:=true
```

### 6.3 Set target position

```bash
rostopic pub /dji2/set_target geometry_msgs/Point -- 0.0 0.0 1.7
```

### 6.4 Set target yaw

```bash
rostopic pub /dji2/set_target_yaw std_msgs/Float64 -- -90.0
```

### 6.5 Set time target

```bash
rostopic pub /dji2/set_target_with_time lrs_msgs_common/ControlTargetWithTime -- 0.0 0.0 1.7 -90.0 3.0
```

## 7. Start With Seperate Programs: Vicon and real flying

### 7.1 Start Vicon on Backpack

Start Vicon for M100:
```bash
roslaunch lrs_vicon mat2.launch
```

### 7.2 Generate TF for depth sensor on Backpack

Static transform to body

```bash
rosrun tf static_transform_publisher 0.15 0.0 -0.04 -1.57 0.0 -1.57  mat2 camera_depth_optical_frame 20
```

### 7.3 Start Controller

Check params:
```bash
rosrun lrs_fly_with_vicon controller __ns:=/dji2 _vicon:=true _mpc:=false
```

Start:
```bash
rosrun lrs_fly_with_vicon controller __ns:=/dji2 _vicon:=true _mpc:=false _start:=true
```



### 7.4 TEST MPC ONE STEP

#### C++

```bash
rosrun lrs_fly_with_vicon controller _mpc:=true _manual:=true _start:=true __ns:=/dji2
```

One step with trajectory xdot=1.0, ydot=1.0

```bash
rostopic pub /dji2/set_target geometry_msgs/Point -- 1.0 1.0 0.0
```

#### Python

```bash
python3 tommytest.py --one-step --roll-pitch --compare
```

### 8.0. LARA: python casadi NMPC - versions (Intresting for Mariusz)

#### python

```bash
python3 dummy_drone_openloop.py 
```

An open-loop example for the NMPC for four rotors control.

```bash
python3 dummy_drone.py --trajectory_type step_z (step_xyz/spline/sinusoidal)
```

Controling the four rotors of a UAV, as control inputs

```bash
python3 dummy_drone_dji100_openloop.py
```

An open-loop example for the NMPC for control inputs: thrust, roll [rad], pitch [rad], yaw [rad/s]

```bash
python3 dummy_drone_dji100.py --trajectory_type step_z (step_xyz)  ---> have not seen the plots for spline and sinusoidal yet
```

Controling thrust, roll [rad], pitch [rad], yaw [rad/s] 

```bash
python3 py_nmpc_dji100_obstacles.py --trajectory_type spline (only spline)  ---> have not seen the plots for spline and sinusoidal yet
```

Controling thrust, roll [rad], pitch [rad], yaw [rad/s] avoid a sphere obstacle.
...
### 0.2. Software Installation and Dependencies: CasADi and ROS
...
(I am going to level with you here the installation is a bit more sinister then this, I only remember that if you want to use ma27 or ma97 (instead of mumps) you need to visit https://licences.stfc.ac.uk/product/coin-hs --- this is my reccomendation for hard problems --- dont get discouraged if its a hard to link
...
### 0.3. Integrating ROS with DJI Matrice 100
...
Set Up ROS Nodes: Implement ROS nodes to handle communication between the NMPC controller and the UAV. This involves publishing control commands and subscribing to telemetry data. http://wiki.ros.org/dji_sdk  
...
## 9. Background Explanation of the Code

### 9.1. Explanation of Various Files and Their Usage
...
### 9.2. How to Tune the Controller?
...
### 9.3. Code Structure Overview

#### 9.3.1. Algorithm 1: NMPC Implementation for UAV
...
#### 9.3.2. Algorithm 2: NMPC Controller Initialization and Operation
...
## 10. NMPC: Simulation and Simulator Tests
...
### 10.1. Testing out the go_to_point trajectory - GO TO A SPECIFIC POINT
...
## 11. NMPC: Simulation and Simulator Tests with STATICAL OBSTACLES
...
Add rviz visualization for Tommy. --- FINISHED inisde pycontroller.py

extra file: saved copy of the last correct rviz visualization
```bash
rviz_koji_radi.py
```
...
State Information
   - `/dji_sdk/attitude` (`geometry_msgs/QuaternionStamped`): This captures the orientation of the drone.
   - `/dji_sdk/local_position` (`geometry_msgs/PointStamped`): This records the drone's position in local coordinates.
   - `/dji_sdk/velocity` (`geometry_msgs/Vector3Stamped`): This logs the velocity of the drone in three dimensions.
   - `/dji_sdk/battery_state` (`sensor_msgs/BatteryState`): This keeps track of the battery status, which is crucial for understanding power consumption and flight time.

Control Inputs
   - `dji_sdk/flight_control_setpoint_generic` (`sensor_msgs/Joy`): This captures the control inputs sent to the drone.
   - `ctrl/roll`, `ctrl/pitch`, `ctrl/thrust`, `ctrl/yaw_rate` (`std_msgs/Float64`): These topics log the individual control commands for roll, pitch, thrust, and yaw rate.

Trajectory and Reference Data
   - `tune/ref_position` (`geometry_msgs/PointStamped`): This logs the reference positions for the drone.
   - `tune/first_yellow` (`geometry_msgs/PointStamped`): This captures the initial reference point in the trajectory.
   - `tune/err_x`, `tune/err_y`, `tune/err_z` (`std_msgs/Float64`): These topics log the position errors in the x, y, and z directions, which are useful for performance analysis.

Extra
   - `velocity` (`geometry_msgs/Vector3`): This topic is used to publish calculated velocities, useful for debugging and analysis.
   - `/tf` (`tf2_msgs/TFMessage`): This captures the transformation data, which is useful for understanding the drone's position and orientation in the world frame.
...
## LARA: current work - SIMULATOR TESTS - DYNAMICAL OBSTACLE - Not working yet.....

Working at the moment on making the obstacles read from the lidar sensor...

ask tommy: how to set up the reading from lidar is your idea ok?

ask tommy: advice about the simulator rviz visualization?

ask tommy: if he wants sphere or something else or multiple ones.

```bash
python3 py_nmpc_dji100_DYNAMIC_OBSTACLE.py --trajectory_type spline --use_nmpc
```
...
```bash
python3 py_nmpc_dji100.py --trajectory_type step_z (step_xyz/spline/sinusoidal)
```
Just a closed loop simulation

Controling thrust, roll [rad], pitch [rad], yaw [rad/s] 

The version where the create_nmpc_solver symbolicaly is only created once.
```bash
python3 py_nmpc_dji100_WORKS.py --trajectory_type step_z
```

THIS HERE IS THE NEW VERSION 25-50 Hz appox. you have to set T = 3; dt = 0.5; N = 30 (getting ~25Hz only for step_z trajectory)

THIS HERE IS THE NEW VERSION 25-50 Hz appox. you have to set T = 3; dt = 0.1; N = 9 (getting ~5Hz only for step_xyz, spline, sinusoidal)
```bash
python3 py_nmpc_dji100_THREADS.py --trajectory_type step_z (step_xyz/spline/sinusoidal)
```
