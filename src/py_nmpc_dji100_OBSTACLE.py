...
class MPCController:
    def __init__(self, initial_speed, initial_thrust, dt=0.1, trajectory_type="go_to_point", use_nmpc="False"): # dt = T/N # Include trajectory_type parameter with default value
        ...
        if trajectory_type == "spline":
            ...
            #    [8.0, 0.0, 2.0]   # End point
                [21.0, 10.0, 6.0],  # Example point
                [25.0, 6.0, 8.0], # Example point
                [28.0, 11.0, 7.0],  # Example point
                [29.0, 8.0, 6.0]   # End point
            ]) 
            ...
            #    [8.0, 0.0, 2.0]   # End point
                ...
                [34.0, 6.0, 8.0], # Example point
                [39.0, 6.0, 8.0],  # Example point
                [44.0, 6.0, 8.0]   # End point
            ]) 

            other_points = other_points2
            # Combine start_point with other_points
            self.points = np.vstack([start_point, other_points])    
        ...
        # OBSTACLES - SPHERE            # 4.0 -2.0 4.1
        self.obstacle_center = np.array([25.0, 6.0, 8.1])  # Obstacle position
        self.obstacle_radius =  0.75 #2.00 #0.75  # Obstacle radius
        ...
        self.safety_distance = 1.25 #0.5  # Safety distance from the obstacle
    ...
    def calculate_objective(self, x_param, u_param, x_desired_param):
        ...
        distance_to_obstacle = sqrt((x_param[0] - self.obstacle_center[0])**2 + 
                                    (x_param[1] - self.obstacle_center[1])**2 + 
                                    (x_param[2] - self.obstacle_center[2])**2)
        eta = 1500.0 #1500.0
    ...
    def create_nmpc_problem(self, use_rk4=False, explicit_euler=True):
        ...
        opts = {
                ...
                'ipopt.tol': 1e-1, #1 #2,       # Tolerance for convergence the bigger the number the tighter the tolerance  # RELAX THIS IF THE OBSTLE CRASHES
                'ipopt.acceptable_tol': 1e-1, #1 #3, # Acceptable tolerance for convergence  # RELAX THIS IF THE OBSTLE CRASHES
                ...
                'ipopt.bound_push': 1e-3,    # 1e-5 was slower but for sure avoids it 
                'ipopt.bound_frac': 1e-3,    # 1e-5 was slower but for sure avoids it 
            }
    ...
    def plot(self, trajectory_type='go_to_point'):
        ...
        plot_sphere_shell(ax3, self.obstacle_center, self.obstacle_radius, self.obstacle_radius + self.safety_distance, outer_color='#FFA500', inner_color='r', outer_alpha=0.3, inner_alpha=0.5)
        ...
        # Plot the obstacle
        plot_sphere(ax3, self.obstacle_center, self.obstacle_radius, color='r', alpha=0.5)

        ax3.set_xlabel('X')
        ax3.set_ylabel('Y')
        ax3.set_zlabel('Z')
        ax3.set_title('Quadrotor 3D Position with Obstacle')
        ax3.legend()
        ax3.grid(True)
