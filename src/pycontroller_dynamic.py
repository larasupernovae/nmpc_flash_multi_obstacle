...
parser = OptionParser()
parser.add_option("", "--controller", action="store", dest="controller_type", type="string", help="Controller type (FLASH, OBSTACLE, MULTI_OBSTACLES, FLASHPOINT, SUPERMAN, DYNAMIC_OBSTACLE)", default="FLASH")
...
parser.add_option("--r_x", "", action="store", dest="r_x", type="float", default=27.0, help='obstacle radius for x')
parser.add_option("--r_y", "", action="store", dest="r_y", type="float", default=8.0, help='obstacle radius for y')
parser.add_option("--r_z", "", action="store", dest="r_z", type="float", default=9.9, help='obstacle radius for z')
...
parser.add_option("", "--obstacle", action="store", dest="rviz_obstacle", type="string", help="Rviz obstacle type (NO_OBSTACLE, OBSTACLE, MULTI_OBSTACLES, DYNAMIC_OBSTACLE)", default="NO_OBSTACLE")
...
elif options.controller_type == 'DYNAMIC_OBSTACLE':
    from py_nmpc_dji100_DYNAMIC_OBSTACLE import MPCController
else:
...
target_obstacle = [options.r_x, options.r_y, options.r_z] # Need for the target reached part of the code
...
def obstacle_callback(r_x, r_y, r_z):
    if hasattr(controller, 'notify_obstacle_center'):
        controller.notify_obstacle_center(r_x, r_y, r_z)
    else:
        rospy.logerr("Controller does not have the method 'notify_obstacle_center'")
...
def target_obstacle_callback(data):
    global new_controller, new_controller_initialised, saved_thrust, saved_speed, target_position, target_reached
    print("NEW TARGET:", dt, options.trajectory_type, data, saved_thrust, saved_speed)
    new_controller = MPCController(saved_speed, saved_thrust, dt=dt, trajectory_type="go_to_point")
    # new_controller = MPCController(speed_adjustment, thrust_adjustment, dt=dt, trajectory_type=options.trajectory_type)
    new_controller.create_nmpc_problem()  # Make sure this is called here
    new_controller.notify_obstacle_center(data.r_x, data.r_y, data.r_z)
    target_obstacle = [data.r_x, data.r_y, data.r_z]
    target_reached = False
    new_controller_initialised = True
...
def timer_callback(event): 
    ...
    elif options.rviz_obstacle == 'DYNAMIC_OBSTACLE':
        vidi_sta_se_desava(marker_pub, covered_traj_points, yref_traj_points, controller.obstacle_center, controller.obstacle_radius, controller.obstacles, covered_id=1, yref_id=3, obstacle_id=2, obstacle_start_id=100)
    else:
...
if __name__ == "__main__":
    ...
    #obstacle_pub = rospy.Publisher("obstacle", Point, latch=False, queue_size=10)
    ...
    target_obstacle_sub = rospy.Subscriber("target_obstacle", Vector3, target_obstacle_callback)
    ...
    #obstacle_sub = rospy.Subscriber("obstacle", Point, obstacle_callback)
