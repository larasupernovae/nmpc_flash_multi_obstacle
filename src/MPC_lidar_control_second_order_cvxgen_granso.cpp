...
#include <dji_sdk_demo/demo_hokuyo_sensor_new.h>
#include "dji_sdk_demo/coordtrans.h"
#include <iostream> 
#include <sstream>
#include <math.h>
...
#include "/home/lrs/lrs_ws/src/dji/Onboard-SDK-ROS/dji_sdk_demo/src/cvxgen/solver.h" 
...
// double desired_horizontal_distance = 6.42; // 0.86 // 3.50 // Desired horizontal distance from the wall   
double max_distance = 10.86;
double desired_velocity = 0.0; // You want the drone to stay still.
double max_control_signal = 1.96; // Maximum control signal value
...
//double desired_distances[max_num_targets] = {6.42, 6.50, 6.60, 6.70, 6.80, 6.90, 7.0, 7.1, 7.2}; 
double desired_distances[max_num_targets] = {6.42, 7.50, 7.60, 7.50, 8.40, 7.00, 7.1, 8.1, 7.2}; 
//double desired_distances[max_num_targets] = {0.56, 0.89, 0.78, 0.87, 0.75, 0.89, 0.78, 0.86, 0.49};
double desired_horizontal_distance = desired_distances[target_index];
...
int main(int argc, char **argv)
{
    ...
    targets[3] = {57.7606029, 16.6839691, 2.3, yaw_radians}; // Target 4
    targets[4] = {57.7606110, 16.6839644, 2.3, yaw_radians}; // Target 5
    targets[5] = {57.7606145, 16.6839469, 2.3, yaw_radians}; // Target 6
    targets[6] = {57.7606204, 16.6839409, 2.3, yaw_radians}; // Target 7
    targets[7] = {57.7606265, 16.6839328, 2.3, yaw_radians}; // Target 8
    targets[8] = {57.7606378, 16.6839194, 2.3, yaw_radians}; // Target 9
  ...
  if (elapsed_time > ros::Duration(0.02)) {
      ...
      if ((current_gps_health > 1) && (target_index <= max_num_targets)) {
            if (target_index < max_num_targets) {      
              ...
              // Update the desired_horizontal_distance using the target_index
              ...
              ROS_INFO("Moving towards target %d: x_new_out = %f, y_new_out = %f, z_new_out = %f, desired_horizontal_distance = %f",
                       target_index + 1, xCmd, yCmd, zCmd, desired_horizontal_distance);              
            }
      } else {
...
void local_position_ctrl(double &xCmd, double &yCmd, double &zCmd)
{
 ...
 // bool asLongasProblem = (std::abs(centeraverageRange - desired_horizontal_distance) > 0.2);
  ...
  if (asLongasProblem) {
      ...
      ros::Duration(2).sleep(); // time you want the drone to wait for stabilization 500ms
  }
  ...
  if (!asLongasProblem && isStablePosition && hasMoreTargets) {
      ...
      ros::Duration(2).sleep(); // Add a delay for stabilization
